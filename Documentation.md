# Практически Проект по Програмни Системи / Софтуерни Платформи 2026

## Тема: Система за управление на ветеринарна клиника за кучета (VetClinic)

**Изготвил (Човек 1):** [Име Фамилия], Ф.Н.: [XXXXX]
**Изготвил (Човек 2):** [Име Фамилия], Ф.Н.: [XXXXX]

**Специалност:** [Специалност]
**Курс / Група:** [Курс / Група]
**Дата:** Април 2026
...
- `Microsoft.EntityFrameworkCore.Tools` (10.0.3) — инструменти за миграции (макар да използвахме `EnsureCreated()`)
...
| Тип | Описание | Автор |
|-----|----------|-------|
| `MainViewModel` | Навигация, превключване на БД | Общо |
| `DogListViewModel` | CRUD за кучета | Човек 1 |
| `OwnerListViewModel` | CRUD за собственици | Човек 1 |
| `VetListViewModel` | CRUD за ветеринари | Човек 1 |
| `SearchFilterViewModel` | Генерично търсене | Човек 1 |
| `GenericListViewModel` | Генеричен списък | Човек 1 |
| `AppointmentListViewModel` | CRUD за прегледи | Човек 2 |
| `MedicalRecordViewModel` | CRUD за мед. записи | Човек 2 |
| `AppointmentSchedulerViewModel` | Насрочване на преглед | Човек 2 |
| `DogHistoryViewModel` | История на прегледи | Човек 2 |
...
### 8.1. Генеричен филтър за търсене (Изискване 4)
...
### 8.2. Генеричен списък с избор (Изискване 5)
...
- `DogListWindow`, `OwnerListWindow`, `VetListWindow` (Човек 1)
- `AppointmentListWindow`, `MedicalRecordWindow` (Човек 2)
...
4. **Разделение на работата** — архитектурата позволи на двама разработчика да работят паралелно: Човек 1 изгради ядрото и управлението на кучета/собственици, Човек 2 — прегледи и медицински записи. Генеричните компоненти се преизползват от двамата.

Основният урок от проекта е, че добрата архитектура и абстракция в началото спестяват огромно количество дублиран код по-нататък. `[Searchable]` и `[Displayable]` атрибутите позволяват добавяне на нов ентитет към търсенето и списъците с нулев допълнителен UI код.
